...
* **容器**主要是解决内存问题，用户只需要将数据放进去或取出来就可以；所以需要**分配器**的支持；

* 有一些操作是容器本身做的，但是还有更多的操作是独立出来的，形成一个个函数（模版函数）即**算法**（排序、查找等）；

* 想要使用算法去操作容器，需要容器和算法之间的桥梁，即**迭代器**（相当于一种泛化的指针）

* **仿函数**：作用相当于函数

* **适配器**：可以使容器、迭代器、仿函数进行一些特殊的转换

* 在**OOP编程**中，都是将数据和操作放到类中，但是这里是将数据放到容器中，操作放到算法中，这就是所谓的GP（**模版编程**）
